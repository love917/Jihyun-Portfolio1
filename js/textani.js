    ...
    var observer = new IntersectionObserver(function(entries) {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          animation_text_1(entry.target);
        } else {
          reset_animation(entry.target);
        }
      });
    }, { threshold: 0.5 });

    var textElements = document.querySelectorAll(".text-anim");
    textElements.forEach(element => observer.observe(element));
  });

  function animation_text_1(element) {
    var textContainer = element.querySelector("div");
    if (!textContainer) {
      console.error("Text container not found");
      return;
    }
    var newText = "";
    for (var i = 0; i < textContainer.innerText.length; i++) {
      newText += "<div>";
      if (textContainer.innerText[i] == " ") {
        newText += "&nbsp;";
      } else {
        newText += textContainer.innerText[i];
      }
      newText += "</div>";
    }
    textContainer.innerHTML = newText;
    var targetsDiv = textContainer.querySelectorAll("div");
    if (targetsDiv.length > 0) {
      gsap.fromTo(targetsDiv, 
        {opacity: 0, y: 90}, 
        {opacity: 1, y: 0, ease: "elastic.out(1.2, 0.5)", stagger: 0.1, duration: 2}
      );
    }
  }
